...
def build_strong_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        augmentation.append(TT.RandomApply([TT.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8))
        augmentation.append(TT.RandomGrayscale(p=0.2))
        augmentation.append(TT.RandomApply([GaussianBlur([0.1, 2.0])], p=0.5))

        randcrop_transform = TT.Compose(
            [
                TT.ToTensor(),
                TT.RandomErasing(p=0.7, scale=(0.05, 0.2), ratio=(0.3, 3.3), value="random"),
                TT.RandomErasing(p=0.5, scale=(0.02, 0.2), ratio=(0.1, 6), value="random"),
                TT.RandomErasing(p=0.3, scale=(0.02, 0.2), ratio=(0.05, 8), value="random"),
                TT.ToPILImage(),
            ]
        )
    ...
    return TT.Compose(augmentation)
