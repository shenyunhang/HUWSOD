...
template <typename T>
__global__ void HierarchyCrossEntropyForward(
    const int nthreads,
    const T* input,
...
template <typename T>
__global__ void HierarchyCrossEntropyBackward(
    const int nthreads,
    const T* grad_output,
...
std::tuple<at::Tensor, at::Tensor> hierarchy_cross_entropy_forward_cuda(
  ...
  at::CheckedFrom c = "hierarchy_cross_entropy_forward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "hierarchy_cross_entropy_forward", [&] {
        HierarchyCrossEntropyForward<scalar_t><<<grid, block, 0, stream>>>(
            output_size,
...
at::Tensor hierarchy_cross_entropy_backward_cuda(
  ...
  at::CheckedFrom c = "hierarchy_cross_entropy_backward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad.scalar_type(), "hierarchy_cross_entropy_backward", [&] {
        HierarchyCrossEntropyBackward<scalar_t><<<grid, block, 0, stream>>>(
            grad.numel(),
