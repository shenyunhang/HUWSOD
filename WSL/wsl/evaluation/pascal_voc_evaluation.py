...
class PascalVOCDetectionEvaluator_WSL(DatasetEvaluator):
    """
    ...
    def evaluate(self):
        ...
        if self._dataset_name == "voc_2012_test":
            return OrderedDict()
        ...
        results_tex = ""
        for ap in aps[50]:
            results_tex += "&{:.1f}".format(ap)
        results_tex += "&{:.1f}\\\\".format(np.mean(aps[50]))
        ...
        for corloc in corlocs[50]:
            results_tex += "&{:.1f}".format(corloc)
        results_tex += "&{:.1f}\\\\".format(np.mean(corlocs[50]))
