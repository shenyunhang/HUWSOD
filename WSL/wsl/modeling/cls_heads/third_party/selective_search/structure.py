...
class HierarchicalGrouping(object):
    ...
    def build_regions(self):
        ...
        for label in self.labels:
            ...
            box = tuple(
                [region_slice[i].start for i in (1, 0)] + [region_slice[i].stop for i in (1, 0)]
            )
    ...
    def merge_region(self, i, j):
        ...
        value = {
            "box": new_box,
            "size": new_size,
            "color_hist": (ri["color_hist"] * ri["size"] + rj["color_hist"] * rj["size"])
            / new_size,
            "texture_hist": (ri["texture_hist"] * ri["size"] + rj["texture_hist"] * rj["size"])
            / new_size,
        }
